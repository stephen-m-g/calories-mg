...
> **Day attribution.** WHOOP's day is physiological (wake-to-wake), not midnight-to-midnight, so a cycle beginning Tuesday morning runs into Wednesday's small hours. Each cycle is pinned to the calendar day it *started*, giving it exactly one owning day and making a closed cycle's total final for that day. Reads always answer the requested date from this cache and never from "whatever cycle the API just returned" — doing the latter was what let an in-progress cycle be shown against the wrong day, so today and yesterday displayed the same number. A consequence worth knowing: between midnight and waking, the new day genuinely has no burn figure yet, and reporting none is the correct answer. Queries `SUM` per day, since an unusual sleep pattern can start two cycles on one calendar day.
