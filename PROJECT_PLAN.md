...
**Status:** In development — all feature phases built. WHOOP and Azure backup are code-complete but unverified against real credentials; both are being validated together in one production-build pass, since the emulator is unusable on the dev machine and each build cycle is slow. See §11. This document is the source of truth for scope and architecture.
...
- ~~Habit learning (typical portion sizes, preferred ground beef leanness, "usual" defaults per food)~~ — **now in scope, built during Phase 7.** See §11 for the design; it turned out to need no ML and no new storage, only statistics over the logs the app already keeps
...
- Mascot / delight animations — attempted, then reverted (see Phase 10 note). Revisit for v2 only with real multi-frame pixel art; a single static pose translated across the screen isn't worth shipping
...
| serving_portions | TEXT nullable | JSON `[{label, gramWeight}]` — known serving sizes (see §6.7) |
| created_at | TEXT | |
...
| quantity_unit | TEXT | matches or converts from food's reference_unit; `each` when counting a portion |
| portion_label | TEXT nullable | which named serving was counted (`banana`, `serving`); null for raw weight |
| portion_gram_weight | REAL nullable | grams per portion **snapshotted at log time**, so a revised upstream average can't move a past entry |
...
| Column | Type | Notes |
|---|---|---|
| id | TEXT PK | the WHOOP cycle id, so re-polling an open cycle overwrites rather than accumulates |
| cycle_date | TEXT | the local calendar day the cycle **started** — see below |
| kilojoules | REAL | raw value from WHOOP |
| calories_burned | REAL | `kilojoules / 4.184`, precomputed for convenience |
| fetched_at | TEXT | |

> **Day attribution.** WHOOP's day is physiological (wake-to-wake), not midnight-to-midnight, so every cycle straddles a boundary and some rule has to pick a side. Each cycle is assigned to the calendar day it spends **most of its length inside** — measured to `end`, or to the present moment while a cycle is still open. An ordinary wake-to-wake cycle puts ~17 of its 24 hours in its start day and lands there; one that runs long crosses over on its own once the majority of it sits in the newer day. The rule settles rather than oscillates, since the majority day's share only grows as a cycle extends. Reads always answer the requested date from this cache and never from "whatever cycle the API just returned". Queries `SUM` per day, since an unusual sleep pattern can start two cycles on one calendar day.
...
A food's `reference_amount` is the **denominator its macros are quoted against** (100 g for both USDA generics and Open Food Facts), not a serving anyone eats. Treating the two as the same thing meant every entry opened at 100 g: a scanned cereal ignored the 39 g serving printed on its own box, and a banana could only be logged as 126 grams rather than as one banana.

Serving sizes are therefore first-class:

- **Sourcing.** USDA exposes portions inline for FNDDS hits and via the detail endpoint for Foundation/SR Legacy (fetched on demand). Open Food Facts declares `serving_quantity` (grams) and `serving_size` (the human string), with a gram figure recovered from the string when the number is missing. Crowd-edited outliers are bounded — a "serving" under 1 g or over 2 kg is discarded rather than silently becoming the default amount.
- **Persistence.** Portions live in `foods.serving_portions` as JSON. They were previously fetched but never stored, so they were dropped the moment a food was cached and every subsequent log fell back to raw weight. Rows cached before this (or first cached from a source that hadn't loaded portions yet) are backfilled in place on next encounter rather than deleted, since logs may already reference them.
- **Selection.** The entry screen offers the food's portions plus its raw unit, ordered so the most human answer leads and becomes the opening selection. Where history exists, the user's own typical amount still outranks the declared serving.
- **Recording.** Counting a portion stores the count with `quantity_unit = 'each'`, plus the portion's label and its gram weight at that moment. Snapshotting the weight matters for the same reason macros are snapshotted: upstream averages get revised, and a past entry must not move.
...
- **Add — Barcode**: scanner → local/Open Food Facts lookup → entry form; on a miss, label photo → OCR-prefilled custom food (barcode saved with it) → entry form
...
- **Settings**: goal mode toggle (fixed intake / deficit — deficit unlocks once WHOOP is connected), calorie goal **or** daily deficit target depending on mode, macro goal inputs, WHOOP connect/disconnect, backup now (disabled, Phase 9), app info
...
| 8 | ~~WHOOP integration~~ — **built, but moved to run last** (see §11: needs a dev build, not Expo Go) |
| 9 | Azure Blob backup — **built out of order, ahead of WHOOP** (see §11) |
| 10 | ~~Mascot / delight animations~~ — **built, then fully reverted.** A single-pose sprite translated across the screen fell well short of what the idea actually needs (real multi-frame pixel art, walk cycles); deferred to a possible v2 rather than shipping a weak version. All code and assets removed |
| 11 | Polish (empty states, error handling), EAS internal distribution build |
| 12 | **WHOOP end-to-end testing** — code from Phase 8 is already in place; this is validating it against a real account and a dev build, done last so it doesn't block everything after it |
...
- **Deficit/surplus chart — deferred, then built in Phase 8**: the zero-centered diverging chart (surplus above the axis, deficit below) waited until WHOOP actually produced deficit data, then shipped as the "Net Calories" metric using `mostNegativeValue` / `noOfSectionsBelowXAxis` / `xAxisLabelsAtBottom` (all verified present in the installed charting library). The anticipated clash with the weight secondary-axis overlay never materialised: the Progress screen had since moved from one combo chart to three dropdown-selected metrics, so net and weight are mutually exclusive by construction rather than by special-casing. Surplus and deficit bars use different colours, since they're opposite outcomes. Net is only plotted for days that have a WHOOP burn figure — eaten-with-no-burn would render as an enormous surplus that never happened.
- **WHOOP integration (Phase 8)**: OAuth 2.0 authorization-code flow via `expo-auth-session`, scopes `read:cycles` + `offline`. **`offline` is not optional** — without it WHOOP issues no refresh token and the connection dies silently at the first access-token expiry. Tokens live in `expo-secure-store` (Keychain/Keystore), never in SQLite: they're live account credentials, and the database is plain-text on disk and destined for cloud backup. `whoop_connection` holds only non-sensitive state. Cycle energy is `score.kilojoule / 4.184`; a cycle whose `score_state` isn't `SCORED` has no kilojoule value, so the cache is left untouched and the last known figure shown rather than a misleading 0. Cached per cycle id so repeated polls of the same in-progress cycle overwrite rather than accumulate, since the value climbs through the day.
- **WHOOP OAuth needs a real build, not Expo Go**: `makeRedirectUri` returns `caloriemate://whoop-callback` in a development/production build but `exp://<lan-ip>:8081/--/whoop-callback` under Expo Go, and WHOOP only accepts exactly-registered redirect URIs. Since the Expo Go address embeds a LAN IP that changes with the network, registering it is impractical — connecting WHOOP requires a development build. This is the first feature in the project that Expo Go cannot run, and it also removes the constraint that kept the project on SDK 54 (see §3).
- **WHOOP moved to the end of the build order**: all of Phase 8's code (OAuth, token storage, cycle polling, deficit mode, the Net Calories chart) is written and type-checked, but untestable until a development build exists — nothing in the rest of the app depends on it, so there's no reason to block subsequent phases on setting one up. The plan's phase *numbers* are left as-is (renumbering everything below WHOOP would just be churn); the phase *order actually executed* moved Azure backup ahead of it instead. This is purely a scheduling change — no WHOOP code was reverted or altered.
- **The Connect WHOOP button had to be gated, not just deferred**: leaving it live under Expo Go was worse than inert — tapping it opened a real browser session to WHOOP's login with a redirect URI Expo Go generates as `exp://<lan-ip>:8081/--/whoop-callback`, which WHOOP has no way to honor, so the flow hangs with no path back into the app. `whoopAuth.ts` now exposes `isWhoopAvailable()`, checking `Constants.executionEnvironment !== ExecutionEnvironment.StoreClient` — the same check `expo-auth-session` uses internally to decide which redirect shape to generate, not a separately-invented condition. Settings shows the row locked (matching the pre-Phase-8 visual pattern) whenever this is false, and `handleConnectWhoop` refuses to call `connectWhoop()` at all in that case. Everything else WHOOP-touches (deficit mode, the Net Calories chart, burn polling) was already self-gated behind `whoopConnected`, which can only become true after a successful OAuth round-trip — impossible under Expo Go regardless — so this one entry point was the only actual hole.
- **Azure backup built ahead of schedule, and it doesn't share WHOOP's blocker**: unlike WHOOP, this needs no OAuth, no browser consent screen, no custom URL scheme — just an authenticated `fetch`-equivalent PUT, since the SAS token lives in the URL's query string. Fully testable in Expo Go. Three pieces verified before building, each against a primary source rather than assumed: (1) Azure's Put Blob REST API needs only `x-ms-blob-type: BlockBlob` plus the raw bytes as the body — confirmed against Microsoft's own REST reference; single-PUT limit is 5,000 MiB, and the database is a few MB. (2) `expo-sqlite`'s `db.serializeAsync()` produces a consistent snapshot via SQLite's native serialize, safe under WAL mode — copying the raw `.db` file directly would risk missing recently-committed data still sitting in a separate `-wal` file. (3) `expo-file-system`'s legacy `uploadAsync()` (imported from the `expo-file-system/legacy` subpath, which re-exports cleanly without deprecation-warning wrapping) supports `httpMethod: 'PUT'` and a `BINARY_CONTENT` upload mode that sends a local file's raw bytes as the request body — sidesteps any question of how React Native's `fetch` would encode a binary body, which was the actual risk in this design.
- **Backup scope is database-only, not photos**: a deliberate choice, not a limitation worth revisiting soon. The database is a few MB and captures everything that matters — logs, weights, settings, cached foods; losing it is the real risk. Photos would need a container-level SAS (broader write permission across many blob names, vs. one) and 100–250MB/year of upload bandwidth per §7's own estimate, for data that's secondary to begin with.
- **SAS URL storage**: kept in `expo-secure-store`, not `.env`, unlike the AI/USDA API keys. A SAS URL is live write-access to the user's actual paid storage account — closer to the WHOOP OAuth tokens in blast radius than to a free-tier, rate-limited API key — so it gets the same secure-storage treatment rather than being bundled into the app binary.
...
- **Habit learning is statistics, not ML**: the app learns typical portions per food, but deliberately trains nothing. With a single user even a favourite food yields only a few hundred samples a year — far too sparse to fit a model without overfitting — and §3's no-backend decision leaves nowhere to train one anyway. What that data *is* enough for is a median, computed live from SQLite in milliseconds with no new tables and nothing running in the background. `getTypicalQuantity` takes the median (not mean, so one holiday portion or mistyped 1500 can't drag the everyday default) of the last 180 days (habits drift), gated at 3+ samples (two entries could be the same meal logged twice). Keyed on normalized food **name** rather than `food_id`, because search ranking can resolve the same word to slightly different USDA rows on different days, which would splinter one real habit across several ids and never reach the threshold. This complements the `previouslyLogged` ranking boost already in `foodRanking.ts`: that learns *which food* you pick, this learns *how much*.
- **WHOOP burn is pinned to the day a cycle started, and reads never trust "the latest cycle"**: found in first-day production use — today and yesterday showed the same calories-burned figure, so past-day deficits were meaningless. Two compounding causes. (1) The read path returned whatever `syncLatestCycle()` fetched whenever the requested day was today, without checking that the fetched cycle actually *belonged* to today; since WHOOP's day runs wake-to-wake, the "current" cycle before waking is still yesterday's, so yesterday's running total was displayed as today's while also being stored under yesterday. (2) Only `limit=1` was ever fetched, so no day the app wasn't opened on got a value, and a cycle that closed after its last poll kept the mid-day total it happened to be read at rather than its final one. Now a rolling 7-day window of cycles is fetched (`start` + `limit`, capped at WHOOP's max of 25) and each is upserted against its own start date, so in-flight days converge on their true totals; and `getBurnedCalories(ymd)` always answers from the cache for exactly the day asked for, with the sync only refreshing that cache rather than deciding the answer. Per-day queries `SUM`, since an unusual sleep pattern can start two cycles in one calendar day. Accepted consequence: between midnight and waking, the new day has no figure yet — which is the honest answer, not a regression. No data repair was needed, since storage had always used the cycle's start date correctly; the bug was entirely in the read path.
- **Cycle→day attribution moved from start-date to majority-overlap**: the start-date rule above was wrong in a way that only showed up in real use. WHOOP closes a cycle when it processes sleep, so a night it doesn't record leaves the cycle open and still climbing — and pinned to its start date, that growing total stayed welded to the day it began. The reported symptom: yesterday's burn kept rising into today while today itself never got a figure at all. A cycle is now assigned to the calendar day holding the largest share of its duration (measured to `now` while open), which leaves ordinary cycles exactly where they were and lets an over-running one hand over to the current day once most of it lives there. Chosen over "the open cycle is always today" — that reads well live but yanks the value back to the start date the moment the cycle closes. Majority overlap can't flip-flop, because a day's share only ever grows as the cycle extends; verified by asserting the assigned day never moves backwards across 72 simulated hours of an open cycle. **Second, independent bug found alongside it:** the sync required `score_state === 'SCORED'`, but a cycle in progress can report `PENDING_SCORE` while already carrying a real running kilojoule total, so the current day was being discarded on that basis too. The gate is now the presence of the number itself, which still rules out the case the original check existed for — storing a missing value as though it were zero burn.
- **Serving sizes are stored, not just fetched (see §6.7)**: reported alongside the WHOOP bug — scanned and searched foods all defaulted to 100 g regardless of the serving on the package, and a banana could only be logged in grams. Root cause was that `reference_amount` (the denominator macros are quoted against) was being used as though it were a serving, and the portion data that would have corrected it was fetched but never persisted — `findOrCacheFood` dropped it, so it vanished the moment a food was cached. Portions now live in `foods.serving_portions`, Open Food Facts' declared `serving_quantity`/`serving_size` are parsed (with crowd-edit outliers bounded out), the entry screen offers them as pickable amounts, and a chosen portion is recorded on the log with its gram weight snapshotted. Existing cached rows are backfilled in place on next encounter rather than deleted, unlike the migration-4 precedent, since logs may already reference them and the correction is additive rather than a bad value needing removal.
- **When a learned portion may override an estimate**: only when nothing better is available, and never silently. Manual entry prefills with it outright, since the alternative is a meaningless `referenceAmount` default of 100g. Voice replaces the amount **only** when Gemini reports `amountStated: false` — a vague "a bowl of rice" is a generic guess the user's own history strictly improves on, whereas a spoken "200 grams" is a fact. Photo never overrides: an estimate made while looking at the actual plate knows more about *this* meal than an average does. In the cases it doesn't apply, the usual amount appears as a one-tap suggestion (suppressed when within 20% of the current value, so it isn't noise), and applied ones say so on the row. Everything stays editable, consistent with the editable-transcript and confidence-badge patterns elsewhere.
- **Barcode flow, and why labels can't be identifiers (Phase 7)**: scan → local `foods` lookup by barcode → Open Food Facts → on a miss, photograph the nutrition label. A label describes a food but cannot *identify* one: it carries no unique code and frequently not even a product name, so it can only ever be the fallback branch, never the lookup. Gemini reads it into a serving size and macros, which prefill the existing custom-food form for correction rather than being saved blind — OCR on small print needs a human check, and unreadable values return null instead of a guess, since a wrong macro is worse than a blank one. The scanned code is stored on the saved row, so the same product resolves locally next time and the label only ever has to be read once. Label images keep more resolution than meal photos (1600px vs 1024px) because downscaling dense small print is what turns "17g" into "1lg".
- **Custom foods were duplicating on every log (fixed)**: found while wiring the barcode fallback. `CreateCustomFood` saved rows with `source_id = NULL`, but search results reference custom foods by their local `id`, so `findOrCacheFood`'s `(source, source_id)` lookup never matched and inserted another copy each time one was logged. A user-created food has no upstream database to point at, so it is now its own source — `createFood` sets `source_id = id` for `source='custom'`, and migration v5 backfills existing rows. Recipes were unaffected, since they resolve through `getOrRefreshRecipeCachedFood` rather than this path.
...
- **Mascot animations built, then fully reverted**: a pixel-art squirrel (tab-switch run, FAB head-peek) was implemented and working, but with only a single static pose available the "run" was a translate-with-bob rather than a real walk cycle — well short of the intent. Rather than ship a weak version, all code (`MascotContext`, `MascotRunOverlay`), the placeholder asset, and the `AddMethodFab`/`BottomTabs`/`FloatingTabBar` integrations were removed; `git diff` confirmed those three files returned to their exact pre-mascot state. Revisit in v2 only with real multi-frame sprite sheets. The architecture note worth keeping: the run needed a context (trigger and visual live in different parts of the tree) while the FAB peek did not (entirely local to the button) — a useful distinction if this is rebuilt.
- **`EXPO_PUBLIC_*` vars must be set in EAS, not `.env`, for cloud builds**: `.env` is gitignored and untracked, and EAS Build uploads the project from its git state — so a cloud build would compile with every key as an empty string. Nothing would error at build time; the app would install and launch, then silently fail at USDA search, voice, photo, and WHOOP, which is a genuinely hard failure to diagnose from the symptom. The values must be registered as EAS environment variables (Expo dashboard → project → Environment Variables, or `eas env:create`) so they're injected at build time. `EXPO_PUBLIC_*` values are inlined into the JS bundle, so this doesn't change the §3 tradeoff — they were always extractable from the binary.
- **Redirect URI is surfaced in-app rather than documented**: WHOOP matches redirect URIs exactly, and `makeRedirectUri`'s output genuinely differs by execution environment — tracing `expo-linking`'s `createURL` shows a production build yields `caloriemate://whoop-callback` (no dev server, so `getHostUri()` returns null), while a dev build attached to a dev server folds the host address into the URI, and Expo Go produces the `exp://` form entirely. Documenting one value would be wrong for two of the three cases, so the Settings screen displays the actual computed value (selectable, long-press to copy) whenever WHOOP is disconnected. Register whatever it shows for the build being used.
- **Privacy policy added (`PRIVACY.md`)**: required for WHOOP developer app registration. Written to describe the app's real behaviour rather than reserve broad rights — notably that there is no backend, so the developer has no access to logged data at all, and that the only WHOOP field read is cycle energy expenditure. Needs a contact email filled in and must be published at a publicly reachable URL (a public GitHub repo view works) before WHOOP registration will accept it.
